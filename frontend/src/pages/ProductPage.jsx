...
function ProductPage() {
  ...
  const extraPrice = useMemo(
    () => selectedOptions.reduce((total, option) => total + Number(option.price_delta || 0), 0),
    [selectedOptions],
  )
  ...
  const validationErrors = useMemo(() => {
    ...
    return product.option_groups.flatMap((group) => {
      ...
      if (group.is_required && selectedCount < group.min_select) {
        return [`Выберите минимум ${group.min_select} в группе "${group.name}"`]
      }

      if (selectedCount > group.max_select) {
        return [`В группе "${group.name}" можно выбрать максимум ${group.max_select}`]
      }
  ...
  function toggleOption(group, optionId) {
    setSelectedOptionsByGroup((currentState) => {
      ...
      if (group.selection_type === 'single') {
        return {
          ...currentState,
          [group.id]: currentSelection.includes(optionId) && !group.is_required ? [] : [optionId],
        }
      }
  ...
  if (error || !product) {
    return (
      <main className="foodclick-page product-page">
        <section className="product-page__shell">
          <p className="section-state section-state--error">{error || 'Товар не найден'}</p>
        </section>
      </main>
    )
  }
        ...
        <div className="product-page__grid">
          <article className="product-page__hero">
            <div className="product-page__image-wrap">
              <img className="product-page__image" src={product.image_url} alt={product.name} />
            </div>
            ...
            <div className="product-page__copy">
              ...
              {selectedOptions.length > 0 ? (
                <p className="product-page__summary">
                  Выбрано: {selectedOptions.map((option) => option.name).join(', ')}
                </p>
              ) : null}
          ...
          <section className="product-page__options">
            ...
            {!product.is_customizable || product.option_groups.length === 0 ? (
              <p className="section-state">У этого товара нет настраиваемых опций.</p>
            ) : (
                      ...
                      {group.options.map((option) => (
                        ...
                        >
                          <div>
                            ...
                            {option.is_default ? <span>По умолчанию</span> : null}
                          </div>
                          <span>
